  ...
  const sections: { emoji: string; title: string; items: string[] }[] = [
    ...
    },
    {
      ...
      items: [
        ...
        'Tap ··· on any player row to expand options: move up/down, pause, set leaving time, skip next game, change gender, or remove.',
        'Paused players are shown with a strikethrough and are skipped by auto-pick until resumed.',
        '⏰ Leaving time: set when a player needs to leave. They are auto-removed from the queue at that time.',
        '⏭ Skip next game: the player sits out the next auto-pick round, then returns to normal.',
        'The queue order determines who plays next — first active (non-paused) players get priority.',
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        `Each ${courtLabel.toLowerCase()} shows who is currently playing and a live timer.`,
        `Set a target game duration in Settings — the timer turns red when time is up so you know when to wrap up.`,
        ...
        `You can add or remove ${courtLabel.toLowerCase()}s in Settings, or rename them individually.`,
        'SUB: swap a player on court with someone from the queue without finishing the match.',
        '→SRV: cycle the serve indicator to track who is currently serving.',
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        'Guests choose their name and gender (M/F) when joining — this affects gender-balanced auto-pick.',
        'Once joined, guests see a live view of the queue, their position, and an estimated wait time.',
        'Guests can tap ··· on their own name in the queue to pause themselves, set a leaving time, or change their gender.',
        'On web, guests are prompted to enable browser notifications so they get an alert when it\'s their turn.',
        'Set a join password in Settings to prevent unauthorised access.',
        '⚡ Power Guest: share a 4-digit PIN with trusted guests so they can manage the queue without being the host.',
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        'Target game duration — set a time limit per match; courts display a countdown and flash red when exceeded.',
        ...
        'Power Guest PIN — set a 4-digit PIN that trusted guests can enter to gain elevated queue controls.',
